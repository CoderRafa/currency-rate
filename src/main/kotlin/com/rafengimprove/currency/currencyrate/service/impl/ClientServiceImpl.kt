...
import com.rafengimprove.currency.currencyrate.model.type.DEFAULT_SORT
...
class ClientServiceImpl(
    ...
    override fun getClientsSortedByLastname(sortType: SortType, pageable: Pageable): Page<ClientDto> {
        return if (sortType == ASC) {
            clientRepository.getAllClientsSortedByLastnameAsc(pageable)
                .map { it.toDto(doINeedExchangeOperations = false) }
        } else {
            clientRepository.getAllClientsSortedByLastnameDesc(pageable)
                .map { it.toDto(doINeedExchangeOperations = false) }
        }
    }
    ...
    override fun findAll(clientFiltersContainer: ClientFiltersContainer, pageable: Pageable): Page<ClientDto> {
        val sortRule = if (clientFiltersContainer.clientSort != null) {
            clientFiltersContainer.clientSort.toSortRule()
        } else {
            DEFAULT_SORT
        }
        return clientRepository.findByFirstNameLikeIgnoreCaseOrLastNameLikeIgnoreCaseOrDateAndTimeCreatedBetween(pageable = PageRequest.of(pageable.pageNumber, pageable.pageSize, sortRule))
            .map { it.toDto() }
    }
}