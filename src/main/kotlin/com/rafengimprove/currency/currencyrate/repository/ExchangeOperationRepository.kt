...
where e.operationType = ?1 and e.fromCurrencyType = ?2 and e.toCurrencyType = ?3 and e.officeEntity.id = ?4"""
    ...
    @Query(
        """select e from ExchangeOperationEntity e
where e.operationType = ?1 and e.fromCurrencyType = ?2 and e.toCurrencyType = ?3 and e.officeEntity.id = ?4 and e.dateAndTimeOfExchange >= ?5"""
    )
    fun topTenExchangeOperationByOfficeSortedBy(
        operationType: OperationType,
        fromCurrencyType: CurrencyType,
        toCurrencyType: CurrencyType,
        id: Long,
    ...
    @Query(
        """select e from ExchangeOperationEntity e
where e.operationType = ?1 and e.fromCurrencyType = ?2 and e.toCurrencyType = ?3 and e.clientEntity.id = ?4 and e.dateAndTimeOfExchange >= ?5"""
    )
    fun topTenExchangeOperationByClientSortedBy(
        operationType: OperationType,
        fromCurrencyType: CurrencyType,
        toCurrencyType: CurrencyType,
        id: Long,
